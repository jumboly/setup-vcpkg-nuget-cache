...
| warm run のはずが全 port 再ビルドになる | GitHub-hosted runner の MSVC または Windows SDK が upstream で更新された (vcpkg の ABI hash が変わる → 全 miss)。月 1 回程度発生。再 build が新しい cache を populate します。 |
